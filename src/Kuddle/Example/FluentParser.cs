...
public static class FluentParser
{
    ...
    static FluentParser()
    {
        ...
        */
        ...
        var number = Terms.Decimal().Then<Expression>(static d => new Number(d));
        ...
        // ( "-" ) unary | primary;
        var unary = primary.Unary((minus, x => new NegateExpression(x))).Named("unary");

        // multiplicative => unary ( ( "/" | "*" ) unary )* ;
        var multiplicative = unary
            .LeftAssociative(
