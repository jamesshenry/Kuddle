...
consecutive `_` characters can appear. Underscores are not allowed _before_ the digits.
That is, `1___2` and `12____` are valid (and both equivalent to just `12`), but
`_12` is _not_ a valid number (it will instead parse as an identifier string),
